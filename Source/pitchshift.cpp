...
void PhaseVocoder::setPitchShift(float in) {
    pitchShift = pow(2.f, in/12.f);
}
...
//        data[s] = ampScale*outBuf[channel][outRdPtr[channel]]/(float)fftPoints;
        data[s] = outBuf[channel][outRdPtr[channel]];
...
//                outBuf[channel][(i+outWtPtr[channel])%bufLen] += window[i]*(td[channel][i]);
                outBuf[channel][(i+outWtPtr[channel])%bufLen] += (orderedInBuf[channel][i]);
            }
